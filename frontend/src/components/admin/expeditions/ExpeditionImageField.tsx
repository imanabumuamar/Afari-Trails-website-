...
type ExpeditionImageFieldProps = {
  expeditionId?: string;
  fieldPath: string;
  label: string;
  src: string;
...
export function ExpeditionImageField({
  expeditionId,
  fieldPath,
  label,
  src,
  ...
  async function uploadFile(file: File) {
    ...
    if (expeditionId) {
      formData.set("expeditionId", expeditionId);
    }

    setUploading(true);
    onStatus(`Uploading ${label}…`);

    const res = await fetch("/api/admin/content/expeditions/upload", {
      method: "POST",
      body: formData,
    });
    ...
    let newSrc = src;
    if (expeditionId) {
      const exp = data.expeditions.find((e) => e.id === expeditionId);
      if (exp) {
        newSrc = resolvePath(exp as unknown as Record<string, unknown>, fieldPath) ?? src;
      }
    } else if (fieldPath.startsWith("allPage.")) {
      newSrc =
        resolvePath(
          data.allPage as unknown as Record<string, unknown>,
          fieldPath.slice("allPage.".length),
        ) ?? src;
    } else {
      newSrc =
